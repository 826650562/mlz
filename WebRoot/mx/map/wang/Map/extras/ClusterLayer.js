dojo.declare("Triman.Map.extras.ClusterLayer", esri.layers.GraphicsLayer, {
  ...
  */
  ...
  onClick: function(e) {
    ...
    if ( singles.length > this._maxSingles ) {
      alert("单个聚合数量超过了 " + this._maxSingles + " 个点。请继续放大地图。");
      return;
    } else {
  ...
  */
  ...
  _clusterCreate: function(p) {
    ...
    // MXZH.log("cluster create, id is: ", clusterId);
  ...
  _showAllClusters: function() {
  	//modify by zl 20130220
    for ( var i = 0, il = this._clusters.length; i < il; i++ ) {
      var c = this._clusters[i];
      this._showCluster(c);
      if(this._isFirst){
      	var e=c.attributes.extent;
      	var extent=new esri.geometry.Extent(e[0], e[1], e[2], e[3], this._sr);
	  	if(i==0) this.zoomExtent=extent;
	  	else this.zoomExtent=this.zoomExtent.union(extent);
	  } 
    }
    this._isFirst=false;
  },
  ...
  _updateClusterGeometry: function(c) {
    ...
    if ( cg.length == 1 ) {
      ...
      MXZH.log("didn't find exactly one CLUSTER GEOMETRY to update: ", cg);
    }
  },
  ...
  _updateLabel: function(c) {
    ...
    if ( label.length == 1 ) {
      // MXZH.log("update label...found: ", label);
      this.remove(label[0]);
      ...
      // MXZH.log("updated the label");
    } else {
      MXZH.log("didn't find exactly one LABEL: ", label);
    }
  },
  ...
  _clusterMeta: function() {
    ...
    MXZH.log("Total:  ", this._clusterData.length);

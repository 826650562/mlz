...
function(declare, domConstruct, query, domStyle, connect, esri) {
    var Widget = declare("Triman.map.extras.HeatmapLayer", [esri.layers.DynamicMapServiceLayer], {
        properties: {},
        heatMap: null,
        // constructor
        constructor: function (properties) {
            ...
            this.globalMax = this.config.max;
			var _self = this;
            // connect on resize
        ...
        setRadius: function (radiusNum){
        	this.config.radius = radiusNum;
        	this.heatMap.set("radius", radiusNum);
        	this.refresh();
        },
        setOpacity: function (opacityNum){
        	this.heatMap.set("opacity", parseInt(255/(100/opacityNum), 10));
        	this.refresh();
        },
        setGradient: function (gradientObj){
        	this.config.gradient = gradientObj;
        	this.heatMap.set("gradient", gradientObj);

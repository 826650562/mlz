...
dojo.declare("Triman.Map.extras.TDTLayer", [esri.layers.TiledMapServiceLayer], {
  copyright: null,
  extent: null,
  tileUrl: null,
  layerInfo: null,
  ...
  constructor: function (type,url, options) {
    ...
    if (options.serviceMode) {
      if (options.serviceMode === "KVP" || options.serviceMode === "RESTful") {
        ...
        MXZH.errorLog("WMTS mode could only be 'KVP' or 'RESTful'");
        return;
      }
    }
  ...
  getTileUrl: function (level, row, col) {
    ...
    if (this.serviceMode === "KVP") {
      tileUrl = this._url.replace("t0", "t"+col%8) + "SERVICE=WMTS&VERSION=" + this.version + "&REQUEST=GetTile" + "&LAYER=" + this._identifier + "&STYLE=" + this._style + "&FORMAT=" + this.format + "&TILEMATRIXSET=" + this._tileMatrixSetId + "&TILEMATRIX=" + level + "&TILEROW=" + row + "&TILECOL=" + col;
    } else if (this.serviceMode === "RESTful") {
      var imagePostfix;
      if (this._formatDictionary[this.format]) {
        imagePostfix = this._formatDictionary[this.format];
      }
      tileUrl = this._url.replace("t0", "t"+col%8) + this._identifier + "/" + this._style + "/" + this._tileMatrixSetId + "/" + level + "/" + row + "/" + col + imagePostfix;
    }
    return tileUrl;
  },
  ...
  _parseCapabilities: function (xmlText) {
    ...
    if (dojo.indexOf(formats, this.format) === -1) {
      MXZH.errorLog("The format " + this.format + " is not supported by the service");
    }
...
// //手动增加了几个参数,modified by Zhangn
  ...
  _getCapabilitiesError: function (err) {
    MXZH.errorLog("Failed to get capabilities xml");
  },
...
/**
 * 天地图类型：Triman.Map.extras.TDTLayer.XXXX，其中XXXX有以下值：
 ...
 */
dojo.mixin(Triman.Map.extras.TDTLayer, {
	VEC_BASE_GCS: "vec_c",
	VEC_ANNO_GCS: "cva_c",

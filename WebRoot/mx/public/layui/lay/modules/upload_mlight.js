/*!
 ...
 @Title: layui.upload_mlight 单文件上传 - 全浏览器兼容版  针对石光公司密讯项目改造。
 @Author: zlj
 @License：MIT
...
layui.define('layer' , function(exports){
  ...
  var Upload_mlight = function(options){
    this.options = options;
  };
  
  //初始化渲染
  Upload_mlight.prototype.init = function(){
    ...
    return elem.each(function(index, item){
      ...
      item.off('change').on('change', function(){
        ...
        //重新定义一个方法
        typeof options.customize === 'function' && options.customize(this, type);
        item.val("");
      });
  ...
  //提交上传
  Upload_mlight.prototype.action = function(input, type){
  ...
  //暴露接口
  exports('upload_mlight', function(options){
    var upload_mlight = new Upload_mlight(options = options || {});
    upload_mlight.init();
  });
});

...
;
layui.define([ "layer", "laytpl", "upload_mlight" ], function(i) {
	var a = "3.5.4 Pro",
		e = layui.jquery,
		t = layui.layer,
		n = layui.laytpl,
	...
	c.prototype.getAllList = function() { //zlj 获取用户通讯录来转换 username 和avater
		return j.friend;
	},
	...
	layui.data.date = function(i) {
		...
		i = (i || "").replace(/&(?!#?[a-zA-Z0-9]+;)/g, "&amp;")
			...
			.replace(/audio\[([^\s]+?)\],[0-9]+/g, function(i) {
				var tokens = i.split(','); // audio[http://cdn.layui.com/xxx/a.amr],500
				return '<div class="layui-unselect layui-layim-audio" layim-event="playAudio" data-duration=' + tokens[1] + ' data-src="' + tokens[0].replace(/(^audio\[)|(\]$)/g, "") + '"><i class="layui-icon">&#xe652;</i><p>音频消息</p></div>'
			})
			...
			.replace(/location\([\s\S]+?\)\[[\s\S]*?\]/g, function(i) {
				var url = (i.match(/location\(([\s\S]+?)\)\[/) || [])[1],
				txt = (i.match(/\)\[([\s\S]*?)\]/) || [])[1];
				return url ? '<div class="layui-unselect layui-layim-video" layim-event="openLocation" data-txt="' + txt + '" data-src="' + url + '"><i class="layui-icon">&#xe652;</i>' + txt + '</div>' : i;
			})
			.replace(a(), "<$1 $2>")
			.replace(a("/"), "</$1>")
			.replace(/\n/g, "<br>");
	...
	var x,
		...
		S = function(i, a, n) {
			return i = i || {}, e.ajax({
					...
					error : function(i, a) {
						window.console && MXZH.log && MXZH.errorLog("LAYIM_DATE_ERROR：" + a)
					}
				})
		},
		j = {
		...
		T = function(i) {
			return b && t.close(b.attr("times")), x && x.hide(), j.mine = j.mine || {}, t.open({
					...
					offset : "rb",
					resize : !1,
					content : '<img src="' + (j.mine.avatar || layui.cache.dir + "css/pc/layim/skin/logo.jpg") + '"><span>' + (i || j.base.title || "即时消息") + "</span>",
		...
		z = function(i) {
			...
			if (a[0]) {
				...
				return "none" === w.css("display") && w.show(), k && t.close(k.attr("times")), 1 !== s.find("li").length || o[0] || (d || w.css("width", 800), s.css({
						height : w.height()
					}).show(), c.css("margin-left", "200px")), o[0] || (s.append(n(v).render(l)), c.append(n(f).render(l)), I(i), E()), q(s.find(".layim-chatlist-" + i.type + i.id)), o[0] || P(), D(i), Q(), C
			}
			l.first = !0;
			var u = C = t.open({
				type : 1,
				...
				success : function(a) {
					w = a, a.css({
						"min-width" : "500px",
						"min-height" : "420px"
					}), I(i), "function" == typeof i.success && i.success(a), Q(), R(a), D(i), P(), O(), layui.each(r.chatChange, function(i, a) {
						a && a(_())
		...
		q = function(i, a) {
			...
			if (a) {
				i.hasClass(o) && q(0 === n ? i.next() : i.prev());
		...
		O = function() {
			...
			a &&
			delete j.message[i.data.type + i.data.id]
		},
		_ = function() {
			if (w) {
		...
		$ = function() {
			...
			if (i.content = a.textarea.val(), "" !== i.content.replace(/\s/g, "")) {
				// if (i.content.length > l)return t.msg("内容最长不能超过" + l + "个字符");   //zlj
		...
		U = function(i) {
			...
			if (j[i.type])
				if ("friend" === i.type) layui.each(j.friend, function(n, l) {
		...
		Q = function() {
			...
			a.focus(), a.off("keydown").on("keydown", function(i) {
				var e = layui.data("layim")[j.mine.id] || {},
					t = i.keyCode;
				if ("Ctrl+Enter" === e.sendHotKey) return void (i.ctrlKey && 13 === t && $());
				if (13 === t) {
		...
		ti = {
					...
					n = function(i) {
						...
						if ("" === a) ti.tab(0 | ti.tab.index); else {
							for (var n = [], l = j.friend || [], s = j.group || [], o = "", d = 0; d < l.length; d++)
								for (var r = 0; r < (l[d].list || []).length; r++) l[d].list[r].username.indexOf(a) !== -1 && (l[d].list[r].type = "friend", l[d].list[r].index = d, l[d].list[r].list = r, n.push(l[d].list[r]));
							for (var c = 0; c < s.length; c++) s[c].groupname.indexOf(a) !== -1 && (s[c].type = "group", s[c].index = c, s[c].list = c, n.push(s[c]));
							if (n.length > 0)
								for (var u = 0; u < n.length; u++) o += '<li layim-event="chat" data-type="' + n[u].type + '" data-index="' + n[u].index + '" data-list="' + n[u].list + '"><img src="' + n[u].avatar + '"><span>' + (n[u].username || n[u].groupname || "佚名") + "</span><p>" + (n[u].remark || n[u].sign || "") + "</p></li>";
							else
			...
			image : function(i) {
				...
				layui.upload_mlight({ //zlj
					...
					customize : function(input, type) { //自定义方法 zlj
						typeof l.m_upload === 'function' && l.m_upload(input, type, j.mine, n.data);
						...
						if (FileReader) { //chrome浏览器处理  
							...
							reader.onload = function(e) {
								...
								showchange(e.target.result);
							};
							reader.readAsDataURL(file);
						}
						//显示图片
						function showchange(mlight_url) {
							MXZH.log("显示图片：", mlight_url);
							ai(n.textarea[0], "img[" + mlight_url + "]");
							$();
						}
					}
				})
			...
			media : function(i) {
				...
				var startRecord = function() {
					selector.data(durationKey, 0);
					Recorder.record({
						start : function() {
							jQuery("._recorder_temp").eq(index_audo).text("准备录音");
							tage=true;
						},
						progress : function(ms) {
							jQuery("._recorder_temp").eq(index_audo).text("正在录音" + ms / 1000 + "s");
							if (tage) {
								selector.data("duration", ms);
								if ((ms / 1000) >= 20) {
									//自动发送
									send();
									tage = false;
				...
				var stopRecord = function() {
					Recorder.stop();tage=false;
					jQuery("._recorder_temp").eq(index_audo).text("录音停止");
				};
				...
				var playRecord = function() {
					Recorder.play({
						progress : function(ms) {
							MXZH.log("Play progressed: " + ms + "ms");
						}
					});
				}
				...
				var getRecorderName = function() {
					var _ms = selector.data(durationKey);
				...
				var uploadCallback = function(resp) {
					...
					try {
						...
						var ms = selector.data("duration");
						if (ms < 1000) {
							layer.msg("录音时间太短,无法发送!");
						} else {
							MXZH.log(obj["oldAddress"])
							ai(l.textarea[0], [ "audio[", obj["oldAddress"], "],", ms ].join(""));
							$();
						}
					} catch (e) {
						layer.msg("录音上传失败,请重试!");
					}
					selector.data("duration", 0);
					jQuery(".getAudio").eq(index_audo).fadeOut(400);
				}
				...
				var uploadRecord = function() {
					var fileFileURL = "/temp_file";
					Recorder.upload({
						method : "POST",
						url : IM.wavUploadUrl,
						audioParam : "file",
						params : {
							myFileName : getRecorderName(),
							fileFileURL : fileFileURL
						},
				...
				var send = function() {
					stopRecord();
					uploadRecord();
				/*	recorder.upload(j.mine, l.data, function(state, e) {
						switch (state) {
						case 'uploading':
						...
						case 'ok':
							//alert(e);
							var path = "/data/fastdfs/data" + e.replace("/M00/", "");
							ai(l.textarea[0], "audio[" + path + "]");
							jQuery("#getAudio").hide();
							break;
						case 'error':
							alert("上传失败");
							break;
				...
				var action = i.data("action"),
					index_audo = jQuery(".layui-icon.layim-tool-audio").index(i);
				switch (action) {
				case "start": // 开始
			...
			_playAudio : function(i) {
				var namesrc = i.data("src");
				...
				Myemb_stop = function(stopstr) {
					...
					amrid.pause();
					i.find("i").attr("status", 'pause').html("&#xe652;");
				}
				...
				function play(playstr) {
					...
					i.find("i").attr("status", 'playing').html("&#xe651;");
			...
			playAudio_ : function(i) {
				//这个播放方法只支持ie 其他浏览器 不支持 wxl
				try {
					var iconImg = i.find("i").attr("status");
					n = function() {
						i.find("i").attr("status", 'pause').html("&#xe652;");
					};
					var namesrc = i.data("src"),
					...
					if (!iconImg || iconImg == 'pause') {
						...
						interAudio = setInterval(function() {
							begin >= ms ? (n(), clearInterval(interAudio)) : begin += 100;
						}, 100);
			...
			openLocation : function(i) { // dl 打开位置
				var src = i.data("src"),
					txt = i.data("txt");
				return src ? (t.close(ti.openLocation.index), void (ti.openLocation.index = t.open({
					type : 1,
					title : "位置--" + txt,
					area : [ "460px", "300px" ],
					maxmin : !0,
	...
	i("layim", new c)
}).addcss("modules/layim/layim.css?v=3.54Pro", "skinlayimcss");